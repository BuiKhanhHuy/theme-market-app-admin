...
import { ProductTypeFilterForm } from '../../components/ProductType';
...
const getColumns = (onReview: (id: number | string) => void, onDelete: (id: number | string) => void): ColumnsType<DataType> => {
  return [
    ...
    },
    {
      ...
      key: 'action',
      render: (_, record) => (
        <TableRowAction id={record.id} onReview={onReview} onDelete={onDelete} editLink={`/product-type/${record.id}/edit`} />
      ),
    },
  ];
}
...
const List: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoadingData(true)
    try {
      const res = await productTypeService.getProductTypeList({
  ...
  const handleDeleteMany = (idList: React.Key[]) => {
    const deleteProductTypeWithIdList = async (productTypeIdList: number[]) => {
      try {
        await productTypeService.deleteProductTypeWithIdList({
    ...
    modal.confirm({
      ...
      okText: 'Delete',
      onOk: () => deleteProductTypeWithIdList(idList as number[])
    })
  }
  ...
  const handleDeleteById = (id: number | string) => {
    const deleteProductType = async (productTypeId: number | string) => {
      try {
        await productTypeService.deleteProductTypeById(productTypeId)
            ...
            <div style={{ marginTop: 20, marginBottom: 15 }}>
              <Collapse
                items={[{
                  ...
                  children: <ProductTypeFilterForm onSubmit={handleFilter} />
                }]}
              />
            </div>
