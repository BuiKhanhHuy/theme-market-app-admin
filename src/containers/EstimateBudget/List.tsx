...
import { EstimateBudgetFilterForm } from '../../components/EstimateBudget';
...
const getColumns = (onReview: (id: number | string) => void, onDelete: (id: number | string) => void): ColumnsType<DataType> => {
  return [
    ...
    },
    {
      ...
      key: 'action',
      render: (_, record) => (
        <TableRowAction id={record.id} onReview={onReview} onDelete={onDelete} editLink={`/estimate-budget/${record.id}/edit`} />
      ),
    },
  ];
}
...
const List: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoadingData(true)
    try {
      const res = await estimateBudgetService.getEstimateBudgetList({
  ...
  const handleDeleteMany = (idList: React.Key[]) => {
    const deleteEstimateBudgetWithIdList = async (estimateBudgetIdList: number[]) => {
      try {
        await estimateBudgetService.deleteEstimateBudgetWithIdList({
          idList: estimateBudgetIdList
        })
    ...
    modal.confirm({
      ...
      okText: 'Delete',
      onOk: () => deleteEstimateBudgetWithIdList(idList as number[])
    })
  }
  ...
  const handleDeleteById = (id: number | string) => {
    const deleteEstimateBudget = async (estimateBudgetId: number | string) => {
      try {
        await estimateBudgetService.deleteEstimateBudgetById(estimateBudgetId)
  ...
  return (
    <>
      <Card title="Estimate budgets">
        <div style={{ minHeight: 360, background: colorBgContainer }}>
          <div>
            {/* Start: TableHeaderAction */}
            <TableHeaderAction addLink='/estimate-budget/add' idList={selectedRowKeys} onDeleteMany={handleDeleteMany} onExport={handleExport} />
            ...
            <div style={{ marginTop: 20, marginBottom: 15 }}>
              <Collapse
                items={[{
                  key: 'FILTER',
                  label: 'Filter',
                  children: <EstimateBudgetFilterForm
