...
const List: React.FC = () => {
  ...
  const handleDeleteMany = (idList: React.Key[]) => {
    const deleteCategoryWithIdList = async (categoryIdList: number[]) => {
      try {
        await categoryService.deleteCategoryWithIdList({
    ...
    modal.confirm({
      ...
      okText: 'Delete',
      onOk: () => deleteCategoryWithIdList(idList as number[])
    })
  }
