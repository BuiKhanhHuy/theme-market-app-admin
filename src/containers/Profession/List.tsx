...
import { ProfessionFilterForm } from '../../components/Profession';
...
const getColumns = (onReview: (id: number | string) => void, onDelete: (id: number | string) => void): ColumnsType<DataType> => {
  return [
    ...
    },
    {
      ...
      width: 100,
      render: (_, record) => (
        <TableRowAction id={record.id} onReview={onReview} onDelete={onDelete} editLink={`/profession/${record.id}/edit`} />
      ),
    },
  ];
}
...
const List: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoadingData(true)
    try {
      const res = await professionService.getProfessionList({
  ...
  const handleDeleteMany = (idList: React.Key[]) => {
    const deleteProfessionWithIdList = async (professionIdList: number[]) => {
      try {
        await professionService.deleteProfessionWithIdList({
          idList: professionIdList
        })
    ...
    modal.confirm({
      ...
      okText: 'Delete',
      onOk: () => deleteProfessionWithIdList(idList as number[])
    })
  }
  ...
  return (
    <>
      <Card title="Professions">
        <div style={{ minHeight: 360, background: colorBgContainer }}>
          <div>
            {/* Start: TableHeaderAction */}
            <TableHeaderAction
              addLink='/profession/add'
