...
import { TimelineFilterForm } from '../../components/Timeline';
...
const getColumns = (onReview: (id: number | string) => void, onDelete: (id: number | string) => void): ColumnsType<DataType> => {
  return [
    ...
    },
    {
      ...
      key: 'action',
      render: (_, record) => (
        <TableRowAction id={record.id} onReview={onReview} onDelete={onDelete} editLink={`/timeline/${record.id}/edit`} />
      ),
    },
  ];
}
...
const List: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoadingData(true)
    try {
      const res = await timelineService.getTimelineList({
  ...
  const handleDeleteMany = (idList: React.Key[]) => {
    const deleteTimelineWithIdList = async (timelineIdList: number[]) => {
      try {
        await timelineService.deleteTimelineWithIdList({
          idList: timelineIdList
        })
    ...
    modal.confirm({
      ...
      okText: 'Delete',
      onOk: () => deleteTimelineWithIdList(idList as number[])
    })
  }
  ...
  const handleDeleteById = (id: number | string) => {
    const deleteTimeline = async (timelineId: number | string) => {
      try {
        await timelineService.deleteTimelineById(timelineId)
      ...
      <Card title="Timelines">
        <div style={{ minHeight: 360, background: colorBgContainer }}>
          <div>
            ...
            <TableHeaderAction addLink='/timeline/add' idList={selectedRowKeys} onDeleteMany={handleDeleteMany} onExport={handleExport} />
