...
import { ApproachFilterForm } from '../../components/Approach';
...
const List: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoadingData(true)
    try {
      const res = await approachService.getApproachList({
  ...
  const handleDeleteMany = (idList: React.Key[]) => {
    const deleteApproachWithIdList = async (approachIdList: number[]) => {
      try {
        await approachService.deleteApproachWithIdList({
          idList: approachIdList
        })
    ...
    modal.confirm({
      ...
      okText: 'Delete',
      onOk: () => deleteApproachWithIdList(idList as number[])
    })
  }
  ...
  const handleDeleteById = (id: number | string) => {
    const deleteApproach = async (approachId: number | string) => {
      try {
        await approachService.deleteApproachById(approachId)
      ...
      <Card title="Approaches">
        <div style={{ minHeight: 360, background: colorBgContainer }}>
          <div>
            ...
            <TableHeaderAction addLink='/approach/add' idList={selectedRowKeys} onDeleteMany={handleDeleteMany} onExport={handleExport} />
